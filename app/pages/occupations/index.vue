<template>
  <div class="p-6 w-full overflow-hidden" style="background: var(--ui-bg); color: var(--ui-text);">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold" style="color: var(--ui-text-highlighted);">Daftar Pekerjaan</h1>
        ...
        <table class="min-w-full table-auto border-collapse" style="color: var(--ui-text);">
          ...
          <tbody>
            <tr v-for="item in occupations" :key="item.id"
              ...
              <td class="px-3 py-3 text-sm whitespace-nowrap">{{ item.member_data?.name }}</td>
              ...
              <td class="px-3 py-3 text-sm whitespace-nowrap">
                <div class="flex justify-center gap-2">
                  <UButton :to="`/occupations/${item.id}`" icon="i-heroicons-pencil-square" size="xs" color="info" variant="soft" label="Edit" />
                  <UButton @click.stop="openDeleteModal(item.id)" icon="i-heroicons-trash" size="xs" color="error" variant="soft" label="Delete" />
                </div>
              </td>
            </tr>
          </tbody>
        ...
        <UCard class="max-w-md w-full mx-4" style="background: var(--ui-bg); color: var(--ui-text); border: 1px solid var(--ui-border);">
          <template #header>
            <div class="flex items-center justify-between">
              ...
              <UButton color="neutral" variant="ghost" icon="i-heroicons-x-mark-20-solid" @click="isDeleteModalOpen = false" />
            </div>
          </template>
...
const tableHeaders = ['ID', 'Anggota', 'Perusahaan', 'Posisi', 'Tahun Mulai', 'Tahun Selesai']
