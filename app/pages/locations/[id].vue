<template>
  <div class="p-6" style="background: var(--ui-bg); color: var(--ui-text);">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold">Edit Lokasi</h1>
      ...
      <form @submit.prevent="save" class="space-y-6">
        ...
        <div class="flex gap-3">
          <UButton type="submit" color="primary" :loading="saving" icon="i-heroicons-check-circle"
            :label="saving ? 'Saving...' : 'Update Location'" />
...
const { fetchById, update, fetchAll, locations, location } = useLocations()

const form = ref({ parent_id: null, name: '', description: '' })
const parents = ref([])
...
onMounted(async () => {
  ...
  await fetchById(currentId)

  if (location.value) {
    form.value.name = location.value.name
    form.value.description = location.value.description
    form.value.parent_id = location.value.parent_id
  }
})

const save = async () => {
  saving.value = true
  await update(currentId, form.value)
