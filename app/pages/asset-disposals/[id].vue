...
const { getOne, update } = useAssetDisposals()
...
onMounted(async () => {
  try {
    ...
    // 🔥 FIX: Cek kalau API nggak ngasih data
    if (!data) {
      serverError.value = "Data disposal tidak ditemukan."
      loading.value = false
      return
    }
    ...
    form.disposal_date = data.disposal_date ?? ""
    form.disposal_type = data.disposal_type ?? ""
    form.value = data.value ?? ""
    form.notes = data.notes ?? ""
  } catch (err) {
    console.error(err)
    serverError.value = "Gagal memuat data asset disposal."
  } finally {
    loading.value = false
  }
})
      ...
      <form @submit.prevent="save" class="space-y-6">
        ...
        <div class="flex items-center gap-3 pt-2">
          <UButton
            ...
            :label="saving ? 'Updating...' : 'Update Disposal'"
          />
          <UButton
            color="gray"
            variant="soft"
