    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          <div class="py-4">
            <p>Yakin mau hapus movement ini?</p>
          </div>
...
const { movements: movementList, fetchAll, remove } = useAssetMovements()
...
const fetchData = async () => {
  try {
    ...
    const assetMap = Object.fromEntries(assets.value.map(a => [a.id, a.name]))
    const locationMap = Object.fromEntries(locations.value.map(l => [l.id, l.name]))
    const userMap = Object.fromEntries(users.value.map(u => [u.id, u.name]))

    movements.value = movementList.value.map(item => ({
      ...item,
      asset_name: assetMap[item.asset_id] || 'Unknown',
      from_location_name: locationMap[item.from_location_id] || 'Unknown',
      to_location_name: locationMap[item.to_location_id] || 'Unknown',
      moved_by_name: userMap[item.moved_by] || 'Unknown'
    }))

  } catch (err) {
    console.error('Gagal memuat asset movements:', err)
    movements.value = []
  }
}
...
const confirmDelete = async () => {
  ...
  try {
    ...
    console.error('Gagal menghapus movement:', err)
  }
}
