...
<template>
  <div class="p-6 w-full overflow-hidden" style="color: var(--ui-text); background: var(--ui-bg);">
    ...
    <div class="flex justify-between items-center mb-6">
      ...
      <UButton to="/news/create" icon="i-heroicons-plus-circle" size="md" color="primary" label="Tambah Berita" />
    </div>
    ...
    <!-- Table -->
    <UCard v-else :ui="{ body: { padding: '' } }" class="relative z-0 overflow-hidden">
      <div class="overflow-x-auto w-full">
        <table class="min-w-full table-auto border-collapse">
          ...
          <tbody>
            <tr v-for="item in news" :key="item.id" class="transition-colors hovered-row">
              ...
              <td class="px-3 py-3 text-sm whitespace-nowrap text-center">
                <div class="flex justify-center gap-2">
                  <UButton :to="`/news/${item.id}`" icon="i-heroicons-pencil-square" size="xs" color="blue"
                    variant="soft" label="Edit" />
                  <UButton @click.stop="handleDelete(item.id!)" icon="i-heroicons-trash" size="xs" color="red"
                    variant="soft" label="Hapus" />
                </div>
              </td>
              <td class="px-3 py-3 text-sm whitespace-nowrap">
                <button @click="toggleShowOnLogin(item)" class="px-3 py-1 rounded text-white text-xs font-semibold"
                  :class="item.show_on_login ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-500 hover:bg-gray-600'">
            ...
            <tr v-if="!news.length">
              <td colspan="5" class="px-3 py-4 text-center text-gray-500 text-sm italic">
