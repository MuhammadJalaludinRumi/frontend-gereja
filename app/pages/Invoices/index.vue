...
// ✅ rename method biar sesuai composable terbaru
const { invoices, loading, fetchAll, remove } = useInvoices()
...
const filteredInvoices = computed(() => {
  ...
  return invoices.value.filter((inv: Invoice) =>
    ...
    inv.organization?.name?.toLowerCase().includes(keyword)
  )
})

const formatDate = (value: string | undefined) => value ? value.split('T')[0] : '-'
const formatCurrency = (value: number | undefined) =>
  value !== undefined ? new Intl.NumberFormat('id-ID', {
    style: 'currency',
    currency: 'IDR',
...
const confirmDelete = async () => {
  ...
  try {
    await remove(selectedInvoice.value.id)
  } catch (err) {
      ...
      <div class="overflow-x-auto w-full">
        <table class="min-w-full table-auto border-collapse" style="color: var(--ui-text);">
          ...
          <tbody style="background: var(--ui-bg);">
            ...
            <!-- Empty State -->
            <tr v-if="!loading && !filteredInvoices.length">
